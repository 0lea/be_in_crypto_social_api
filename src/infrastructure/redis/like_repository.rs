...
pub struct RedisLikeRepository {
    client: Arc<Client>,
    conn: ArcSwap<Option<MultiplexedConnection>>,
    config: Config,
}
...
impl RedisLikeRepository {
    pub async fn new(client: Arc<Client>) -> Arc<Self> {
        ...
        // Background monitor per il self-healing
        let repo_clone = Arc::clone(&repo);
        tokio::spawn(async move {
            repo_clone.monitor_connection().await;
        });
    ...
    async fn monitor_connection(&self) {
        ...
        loop {
            ...
            // Carichiamo la connessione attuale (molto veloce, lock-free)
            let current_conn = self.conn.load();

            let needs_reconnect = match &**current_conn {
                Some(conn) => {
            ...
            if needs_reconnect {
                tracing::warn!("Redis disconnected. Attempting background reconnect...");
                if let Ok(new_conn) = self.client.get_multiplexed_async_connection().await {
                    self.conn.store(Arc::new(Some(new_conn)));
...
impl LikeCacheRepository for RedisLikeRepository {
    async fn increment(&self, c_type: &ContentType, c_id: &ContentId) -> Result<u64, DomainError> {
        let Some(conn) = &**self.conn.load() else {
            return Err(DomainError::CacheOffline);
        };
