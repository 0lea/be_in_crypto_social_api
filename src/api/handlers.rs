...
pub async fn delete_unlike(
    ...
    let res = service
        .remove_like(&user_id, &c_type, &c_id)
        .await
        .map_err(|e| ApiError(e, ctx.id))?;
...
pub async fn get_status(
    ...
    let res = service
        .get_like_status(&user_id, &c_type, &c_id)
        .await
        .map_err(|e| ApiError(e, ctx.id))?;
...
pub async fn get_count_batch(
    ...
    let res = service
        .get_likes_count_batch(payload.items)
        .await
        .map_err(|e| ApiError(e, ctx.id))?;
...
pub async fn get_status_batch(
    ...
    let res = service
        .get_batch_status(&user_id, payload.items)
        .await
        .map_err(|e| ApiError(e, ctx.id))?;
...
pub async fn get_top_likes(
    ...
    let response = service
        .get_top_likes(c_type, query.window, limit)
        .await
        .map_err(|e| ApiError(e, ctx.id))?;
    Ok(Json(response))
}
